...
/// on a normal hosted CI runner. It's tagged "Hardware" so it's excluded from this repo's own
/// build/PR pipeline (see .github/workflows/contract-tests.yaml), which has no CI infrastructure
/// that reaches a real device. It runs instead from sdk-dotnet's nightly workflow
/// (contract-tests-nightly.yml), which checks out this repo read-only and uses its own device
/// runners. To run it manually on a machine with a device attached:

...
/// <summary>
/// Plain-text, append-only sink for CI notices (breaking-change failures, additive notices, and
/// the tested SDK version), read back by contract-tests.yaml's Slack-notification step - and, in
/// sdk-dotnet, contract-tests-nightly.yml's own Slack step. No-ops unless the
...
/// "# heading") and Slack's mrkdwn ("*bold*", no headings) aren't compatible, so each Slack step
/// builds its own formatting from this raw text instead of trying to reuse either dialect.
/// </summary>
internal static class CiNoticeSink
{
